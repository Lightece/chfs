...
//      std::cout << "metadata/manager.cc/free_inode_cnt" << std::endl;
    // read inode allocation bitmap
    std::vector<u8> buffer(bm->block_size());
    usize cnt = 0;
//    std::cout << "n_bitmap_blocks:" << n_bitmap_blocks << std::endl;
    for(int i = 0; i < this->n_bitmap_blocks; ++i){
      auto read_res = bm->read_block(1 + n_table_blocks, buffer.data());
      // print buffer.data()
      if(read_res.is_err()) return ChfsResult<u64>(read_res.unwrap_error());
      if(i < n_bitmap_blocks - 1){
        Bitmap bitmap(buffer.data(), bm->block_size());
        cnt += bitmap.count_zeros_to_bound(bm->block_size() * KBitsPerByte);
      }
      else {
        // last bitmap block, partial read
        Bitmap bitmap(buffer.data(), bm->block_size());
        cnt += bitmap.count_zeros_to_bound(max_inode_supported - i * (bm->block_size() * KBitsPerByte));
      }
    }
    std::cout << "metadata/manager.cc/free_inode_cnt: count=" << cnt << std::endl;
    return cnt;
  }
